...
async def client_main(websocket, data):
    ...
    message = "Q：" + data  # Go to Alipay to analyze my spending in annual
    ...
    try:
        while True:
            ...
            if len(tapped_button_str) == 1: # =1代表初始，即没有点击过控件
                ...
                object_message_humanword = f'Q：{tasck_str}{tapped_button_string}，{help_str}当前界面有以下按钮：{humanword}{invalid_button_string}'
            print(object_message_humanword)
            await websocket.send_text(object_message_humanword)
            ...
            if isinstance(response, str):
                ...
                if op == 0:
                    print("操作完成")
                    await websocket.send_text("操作完成") # 发送中间数据
                    return 0
                print("\n*---------------------------- √ ONE STEP COMPLETED ----------------------------*\n\n\n")
